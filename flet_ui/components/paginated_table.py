...
class PaginatedTable(ft.Column):
    def __init__(self, columns, fetch_data_callback, build_row_callback, page_size=10):
        super().__init__()
        self.expand = True
        ...
        self.build_row_callback = build_row_callback
        ...
        self.btn_prev = ft.FilledButton("Anterior", icon=ft.icons.Icons.ARROW_BACK, on_click=self.prev_page, disabled=True, style=ft.ButtonStyle(bgcolor="#444444", color="white", shape=btn_shape))
        self.btn_next = ft.FilledButton("Siguiente", icon=ft.icons.Icons.ARROW_FORWARD, on_click=self.next_page, disabled=True, style=ft.ButtonStyle(bgcolor="#444444", color="white", shape=btn_shape))
    ...
    def load_page(self):
        ...
        # Obtenemos 1 registro más para saber si hay página siguiente
        data = self.fetch_data_callback(skip, self.page_size + 1)
        
        has_next = len(data) > self.page_size
        display_data = data[:self.page_size]
        
        self.table.rows = [self.build_row_callback(item) for item in display_data]
